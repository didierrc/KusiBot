...
def test_ut07_get_conversation_for_user_some_conversations(mock_conv_repo, mock_msg_repo ,dashboard_service):
    ...
    mock_conv_repo.get_last_conversation_by_user_id.return_value = Conversation(
        id = 1, user_id = 1
    )
    ...
    result = dashboard_service.get_conversation_for_user(1)
    
    assert result["conversation"] is not None
    assert result["conversation"]["id"] == 1
    assert result["conversation"]["finished_at"] is None
...
def test_ut08_get_conversation_for_user_no_conversations(mock_conv_repo, dashboard_service):
    
    # Mocking the ConversationRepository call
    mock_conv_repo.get_last_conversation_by_user_id.return_value = None
